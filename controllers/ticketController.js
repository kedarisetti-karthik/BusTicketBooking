  ...
  await Ticket
    ...
    .exec((err, response) => {
      if (err) {
        res
          .status(500)
          .json({ "message": "Unable to fetch Records from DB" })
      } else {
        res
          .status(200)
          .json(response)
...
module.exports.viewById = async function (req, res, next) {
  ...
  if (result) {
    let ticket;
    try {
      ticket = await Ticket.findById(req.params.id).select('name email phone age gender');
...
module.exports.admin = async function (req, res, next) {
  try{
  const ticket = await Ticket.update({}, {$set: {isOpen: true}, $unset: { name: "", email: "",phone:"" ,date:"",age:"",gender:""}}, { multi: true });
...
module.exports.updateById = async function (req, res, next) {
  ...
  if (!oldticket) return res.status(404).send("The ticket with the given ID was not found.");
  
   ticket = await Ticket.findByIdAndUpdate(
    req.params.id,
    {
      name: req.body.name!=null?req.body.name:oldticket.name,
      isOpen: req.body.name!=null?req.body.isOpen:false,
